...
#define LOCTEXT_NAMESPACE "ErrorHandlingEditor_ErrorCategoryEnumDetails"
...
public:
	...
	virtual void SetText(const int32 InIndex, const FText& InText) override
	{
		...
		ErrorHandling::SetErrorValueDisplayName(*TargetEnum, EnumeratorIndex, InText);
	}
...
public:
	...
	virtual FText GetToolTipText() const override
	{
		return FText::Format(
			LOCTEXT("TooltipFmt_ErrorMessage",
				"Error Message{0}Message displayed when logging this error.{0}WARNING: Not available in shipping builds."),
	...
	virtual void SetText(const int32 InIndex, const FText& InText) override
	{
		...
		ErrorHandling::SetErrorValueMessage(*TargetEnum, EnumeratorIndex, InText);
	}
...
void FECCustomization_ErrorCategory::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	ResultsBuilder = MakeShareable(new FECErrorCategoryNodeBuilder(*TargetErrorCategory));

	IDetailCategoryBuilder& InputsCategory = DetailBuilder.EditCategory(TEXT("Results"), LOCTEXT("CategoryLabel_Results", "Results"));
	InputsCategory.AddCustomBuilder(ResultsBuilder.ToSharedRef());
}

void FECCustomization_ErrorCategory::RequestRefresh()
{
	ResultsBuilder->RequestRefresh();
}
...
void FECErrorCategoryNodeBuilder::GenerateHeaderRowContent(FDetailWidgetRow& NodeRow)
{
	TSharedRef<SWidget> AddButton = PropertyCustomizationHelpers::MakeAddButton(FSimpleDelegate::CreateSP(this, &FECErrorCategoryNodeBuilder::AddEntry),
		LOCTEXT("AddErrorTooltip", "Add Error"));
	
	NodeRow
		.FilterString(LOCTEXT("FilterString_Errors", "Errors"))
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("Label_HeaderRow", "Error"))
...
void FECErrorCategoryNodeBuilder::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	for (int32 EnumIdx = 0; EnumIdx < NumValues; ++EnumIdx)
	{
		...
		TSharedRef<SWidget> RemoveButton = PropertyCustomizationHelpers::MakeDeleteButton(FSimpleDelegate::CreateSP(this, &FECErrorCategoryNodeBuilder::RemoveEntryAtIndex, EnumIdx),
			LOCTEXT("Tooltip_RemoveError", "Remove Error"));
...
{
	ErrorHandling::AddErrorValueToCategory(*TargetErrorCategory, NextId++);
}
...
{
	ErrorHandling::RemoveErrorValueFromCategory(*TargetErrorCategory, Index);
}
