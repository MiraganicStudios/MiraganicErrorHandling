...
#define LOCTEXT_NAMESPACE "ErrorHandlingEditor_K2NodeSwitchResult"
...
{
	return LOCTEXT("K2Node_SwitchResult_ToolTip", "Selects an output that matches the input value");
}

FText UECK2Node_SwitchResult::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return LOCTEXT("K2Node_SwitchResult_Title", "Switch on Result");
}
...
void UECK2Node_SwitchResult::ValidateNodeDuringCompilation(FCompilerResultsLog& MessageLog) const
{
	...
	for (const FECResult& PinResultCode : PinResultCodes)
	{
		...
		{
			MessageLog.Error(*LOCTEXT("SwitchResult_SuccessCase", "@@ contains invalid case(s).").ToString(), this);
			return;
		}
	}
}
...
void UECK2Node_SwitchResult::CreateCasePins()
{
	...
	SuccessPin->PinFriendlyName = LOCTEXT("FriendlyName_SuccessPin", "Success");
