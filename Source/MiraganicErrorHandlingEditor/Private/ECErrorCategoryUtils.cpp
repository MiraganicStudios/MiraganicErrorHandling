...
void ErrorHandling::BroadcastPreChange(UECErrorCategory& ErrorCategory)
{
...
void ErrorHandling::BroadcastPostChange(const UECErrorCategory& ErrorCategory,
	const TArray<TPair<FName, int64>>& OldNames,
	bool bResolveData
)
{
...
void ErrorHandling::SetErrorValueDisplayName(UECErrorCategory& Category,
	int32 Idx,
	...
	)
{
	...
	ErrorHandling::BroadcastPreChange(Category);
	...
	ErrorHandling::BroadcastPostChange(Category, TArray<TPair<FName, int64>>(), false);
}

void ErrorHandling::SetErrorValueMessage(UECErrorCategory& Category, int32 Idx, const FText& NewMessage)
{
...
void ErrorHandling::AddErrorValueToCategory(UECErrorCategory& Category, int64 NewCode)
{
	...
	ErrorHandling::BroadcastPostChange(Category, OldNames);

	Category.MarkPackageDirty();
}

void ErrorHandling::AddSuccessValueToCategory(UECErrorCategory& Category)
{
	TArray<TPair<FName, int64>> OldNames;
	CopyErrorValuesWithoutMax(OldNames, Category);
	TArray<TPair<FName, int64>> Names = OldNames;
	...
	ErrorHandling::SetErrorValueMessage(Category, 0, LOCTEXT("Message_Success", "Successful result."));

	Category.MarkPackageDirty();
}

void ErrorHandling::RemoveErrorValueFromCategory(UECErrorCategory& Category, int32 Idx)
{
...
void ErrorHandling::CopyErrorValuesWithoutMax(TArray<TPair<FName, int64>>& OutEnumPairs, const UECErrorCategory& Category)
{
...
void ErrorHandling::FindAllErrorCategories(TArray<const UEnum*>& OutErrorCategories)
{
